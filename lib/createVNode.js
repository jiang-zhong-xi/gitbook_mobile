function create(summary) {
  ...
  titles.reduce((pre, next, index) => {
    ...
    if (pre.level < next.level) { // 如果当前标题等级小于上个标题,则往下添加
      ...
      recall = getGrand(vnode, next, vnode)
...
function getGrand(obj, current, parent) {
  if (obj.levels !== current.levels) {
    obj.children && obj.children.forEach(item => {
      getGrand(item, current, obj)
    })
  } else {
    return parent
  }
